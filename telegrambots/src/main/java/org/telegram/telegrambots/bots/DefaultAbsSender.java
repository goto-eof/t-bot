...
public abstract class DefaultAbsSender extends AbsSender {
	...
	protected DefaultAbsSender(DefaultBotOptions options) {
		...
		if (configFromOptions != null) {
			...
			this.requestConfig = RequestConfig.copy(RequestConfig.custom().build()).setSocketTimeout(SOCKET_TIMEOUT).setConnectTimeout(SOCKET_TIMEOUT).setConnectionRequestTimeout(SOCKET_TIMEOUT).build();
		}
	}
	...
	public Boolean execute(SetChatPhoto setChatPhoto) throws TelegramApiException {
		...
		try {
			...
			InputStream is = setChatPhoto.getPhotoStream();
			if (setChatPhoto.getPhoto() != null) {
				builder.addBinaryBody(SetChatPhoto.PHOTO_FIELD, setChatPhoto.getPhoto());
			} else if (is != null) {
				builder.addBinaryBody(SetChatPhoto.PHOTO_FIELD, is, ContentType.APPLICATION_OCTET_STREAM, setChatPhoto.getPhotoName());
			}
			HttpEntity multipart = builder.build();
			httppost.setEntity(multipart);
			if (is != null) {
	...
	private void addInputData(MultipartEntityBuilder builder, InputMedia media, String mediaField, boolean addField) throws JsonProcessingException {
		if (media.isNewMedia()) {
			InputStream is = media.getNewMediaStream();
			if (media.getMediaFile() != null) {
				...
				builder.addBinaryBody(media.getMediaName(), is, ContentType.APPLICATION_OCTET_STREAM, media.getMediaName());
			}
			if (is != null) {
				try {
					is.close();
	...
	private void addInputFile(MultipartEntityBuilder builder, InputFile file, String fileField, boolean addField) {
		if (file.isNew()) {
			InputStream is = file.getNewMediaStream();
			if (file.getNewMediaFile() != null) {
				...
				builder.addBinaryBody(file.getMediaName(), is, ContentType.APPLICATION_OCTET_STREAM, file.getMediaName());
			}
			if (is != null) {
				try {
					is.close();
